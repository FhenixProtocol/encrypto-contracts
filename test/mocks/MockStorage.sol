...
contract TaskManager is ITaskManager {
    ...
    mapping(uint256 => uint256) public daStorage;
    mapping(uint256 => bool) public inDaStorage;

    function insertIntoStorage(uint256 ctHash, uint256 value) public {
        daStorage[ctHash] = value;
        inDaStorage[ctHash] = true;
    }
    ...
    function unaryOperation(
        ...
        if (operation == Utils.functionIdToString(FunctionId.random)) {
            insertIntoStorage(ctHash, uint256(blockhash(block.number - 1)));
            return;
        }
        if (operation == Utils.functionIdToString(FunctionId.trivialEncrypt)) {
            insertIntoStorage(ctHash, input);
            return;
        }
        if (operation == Utils.functionIdToString(FunctionId.cast)) {
            insertIntoStorage(ctHash, daStorage[input]);
            return;
        }
        if (operation == Utils.functionIdToString(FunctionId.not)) {
            bool inputIsTruthy = daStorage[input] == 1;
            insertIntoStorage(ctHash, inputIsTruthy ? 0 : 1);
            return;
        }
        if (operation == Utils.functionIdToString(FunctionId.square)) {
            insertIntoStorage(ctHash, daStorage[input] * daStorage[input]);
            return;
        }
        revert InvalidUnaryOperation(operation);
    }

    function twoInputOperation(
        uint256 ctHash,
        string memory operation,
        uint256 input1,
        uint256 input2
        ...
        if (operation == Utils.functionIdToString(FunctionId.sub)) {
            insertIntoStorage(ctHash, daStorage[input1] - daStorage[input2]);
            return;
        }
        ...
        if (operation == Utils.functionIdToString(FunctionId.min)) {
            insertIntoStorage(
                ctHash,
                daStorage[input1] < daStorage[input2]
                    ? daStorage[input1]
                    : daStorage[input2]
            );
            return;
    ...
    function threeInputOperation(
        ...
        if (operation == Utils.functionIdToString(FunctionId.select)) {
            insertIntoStorage(
                ctHash,
                daStorage[input1] == 1 ? daStorage[input2] : daStorage[input3]
            );
            return;
        }
    ...
    function sendEventCreated(
        ...
        if (inputs.length == 1) {
            ...
            // MOCK
            unaryOperation(ctHash, operation, inputs[0]);
        } else if (inputs.length == 2) {
            emit TaskCreated(ctHash, operation, inputs[0], inputs[1], 0);

            // MOCK
            twoInputOperation(ctHash, operation, inputs[0], inputs[1]);
        } else {
            emit TaskCreated(
                ctHash,
                operation,
    ...
    function createDecryptTask(uint256 ctHash, address requestor) public {
        ...
        // MOCK
        decryptOperation(ctHash, requestor, msg.sender);
    }

    function sealoutputOperation(
        uint256 ctHash,
        bytes32 publicKey,
        address requestor,
        address sender
        ...
        string memory sealedOutput = string(
            abi.encodePacked(publicKey, daStorage[ctHash])
        );
    ...
    function createSealOutputTask(
        ...
        // MOCK
        sealoutputOperation(ctHash, publicKey, requestor, msg.sender);
    }
    ...
    function insertKeyIntoDaStorage(uint256 ctHash, uint256 value) public {
        insertIntoStorage(ctHash, value);
    }

    function verifyKeyInStorage(uint256 ctHash) public view {
        return inDaStorage[ctHash];
    }
